...
def start_timer():
    ...
    if reps % 8 == 0:
        timer_label.config(text="Long Break", font=(FONT_NAME, 45), fg=RED, bg=YELLOW)
        count_down(long_break_sec)
    
    elif reps % 2 == 0:
        timer_label.config(text="Short break", font=(FONT_NAME, 45), fg=PINK, bg=YELLOW)
        count_down(short_break_sec)
    
    else:
        timer_label.config(text="Work", font=(FONT_NAME, 45), fg=GREEN, bg=YELLOW)
        count_down(work_sec)

# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(count):
    #reconfiguring in a canvas
...
checkmarks_label = Label(fg=GREEN, font=(20), bg=YELLOW)
